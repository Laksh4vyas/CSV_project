...
create_table()  # DB Table Creation
...
if uploaded_file:
    try:
        ...
        # 🔮 Prediction Tool (XGBoost)
        st.markdown("## 🤖 Predict Target Column (XGBoost)")
        if len(df.columns) >= 2:
            target_column = st.selectbox("🎯 Select Target Column", df.columns)
            if target_column:
                X = df.dropna().drop(columns=[target_column])
                y = df.dropna()[target_column]
            ...
            st.warning("⚠️ Not enough columns to run prediction.")
